...
actual class Extractor {
    ...
    actual fun mergeAudioVideoDownload(filePath: String): DownloadProgress {
        val command =
            listOf(
                ...
                "1:a:0",
                "-shortest",
                "$filePath-VlynnMusic.mp4",
            ).joinToString(" ")

        if (FileSystem.SYSTEM.exists("$filePath-VlynnMusic.mp4".toPath())) {
            FileSystem.SYSTEM.delete("$filePath-VlynnMusic.mp4".toPath())
        }
    ...
    actual fun saveAudioWithThumbnail(
        ...
        try {
            ...
            if (FileSystem.SYSTEM.exists("$filePath-vlynnmusic.mp3".toPath())) {
                FileSystem.SYSTEM.delete("$filePath-vlynnmusic.mp3".toPath())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
