...
// desktopApp — JVM application module for VlynnMusic Desktop.
//
...
// Append VlynnMusic-specific keys to Conveyor's generated config file and
...
tasks.named<hydraulic.conveyor.gradle.WriteConveyorConfigTask>("writeConveyorConfig") {
    ...
    doLast {
        destination.get().asFile.appendText(
            """
            |app.fsname = vlynnmusic
            |app.display-name = VlynnMusic
            |app.rdns-name = com.vlynn.music
            |
...
compose.desktop {
    application {
        mainClass = "com.vlynn.music.MainKt"
                ...
                val macExtraPlistKeys =
                    ...
                    <array>
                        <dict>
                            ...
                            <string>com.vlynn.music.deeplink</string>
                            <key>CFBundleURLSchemes</key>
                            <array>
                                <string>vlynnmusic</string>
                            </array>
                        </dict>
                    </array>
                    """.trimIndent()
...
tasks.register("packageConveyorAppImage") {
    ...
    doLast {
        ...
        val iconDst = appDir.resolve("vlynnmusic.png")
        if (!iconDst.exists() && iconSrc.exists()) {
            FileUtils.copyFile(iconSrc, iconDst)
        }

        val versionName = libs.versions.version.name.get()
        val desktopFile = appDir.resolve("vlynnmusic.desktop")
        desktopFile.writeText(
            """[Desktop Entry]
            |Type=Application
            |Version=1.0
            |Name=VlynnMusic
            |Comment=VlynnMusic v$versionName - FOSS YouTube Music Client
            |Exec=bin/vlynnmusic %u
            |Icon=vlynnmusic
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=com-vlynn-vlynnmusic-MainKt
            |MimeType=x-scheme-handler/vlynnmusic;
            |
            """.trimMargin(),
        )
        ...
        appRun.writeText(
            ...
            |if [ ! -f "${'$'}ICON_DIR/vlynnmusic.png" ] || [ "${'$'}HERE/vlynnmusic.png" -nt "${'$'}ICON_DIR/vlynnmusic.png" ]; then
            |    mkdir -p "${'$'}ICON_DIR"
            |    cp "${'$'}HERE/vlynnmusic.png" "${'$'}ICON_DIR/vlynnmusic.png"
            ...
            |sed "s|Exec=bin/vlynnmusic|Exec=${'$'}APPIMAGE_PATH|" "${'$'}HERE/vlynnmusic.desktop" > "${'$'}DESKTOP_DIR/com-vlynn-vlynnmusic-MainKt.desktop"
        ...
        // Conveyor's launcher lives at output/bin/vlynnmusic (lowercase).
        val appExecutable = appDir.resolve("bin/vlynnmusic")
...
tasks.register("buildLinuxAppImage") {
    group = "distribution"
    description = "Full VlynnMusic Desktop Linux AppImage build pipeline (vlcSetup → conveyor → AppImage)."
...
tasks.register("buildMacZipAmd64") {
    group = "distribution"
    description = "Full VlynnMusic Desktop macOS Intel .zip pipeline (vlcSetup → conveyor)."
    dependsOn(conveyorMakeMacZipAmd64)
}

tasks.register("buildMacZipAarch64") {
    group = "distribution"
    description = "Full VlynnMusic Desktop macOS Apple Silicon .zip pipeline (vlcSetup → conveyor)."
...
tasks.register("buildWindowsMsix") {
    group = "distribution"
    description = "Full VlynnMusic Desktop Windows .msix pipeline (vlcSetup → conveyor)."
