...
class VlynnMusicLyrics {
    ...
    private val baseUrl = "https://api-lyrics.vlynnmusic.org/v1/"
    ...
    private fun createClient() =
        HttpClient(getEngine()) {
            ...
            install(HttpCache)
            install(CurlLogger) {
                logger = { Logger.d("VlynnMusicLyrics", it) }
            }
            install(HttpSend) {
                maxSendCount = 100
            }
            ...
            defaultRequest {
                url("https://api-lyrics.vlynnmusic.org/v1")
            }
            if (proxy != null) {
                engine {
                    proxy = this@VlynnMusicLyrics.proxy
                }
            }
        }
    ...
    private fun HttpRequestBuilder.buildDefaultHeaders(
        ...
        headers {
            ...
            header(HttpHeaders.UserAgent, "VlynnMusicLyrics/1.0")
