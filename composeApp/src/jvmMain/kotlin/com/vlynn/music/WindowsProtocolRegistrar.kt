...
/**
 * Registers the "vlynnmusic://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\vlynnmusic
 *     (Default) = "URL:VlynnMusic Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "vlynnmusic"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering vlynnmusic:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:VlynnMusic Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\VlynnMusic.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/VlynnMusic.exe
        // So we go: java.home → parent (runtime) → parent (app) → VlynnMusic.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "VlynnMusic.exe")
            if (exeFile.exists()) {
