...
suspend fun fetchExternalAsset(
    ...
    // Check cache first to avoid rate limiting and dropping Discord RPC
    externalAssetCache[imageUrl]?.let { return it }
    ...
    return runCatching {
        ...
        list.firstOrNull()?.externalAssetPath?.let { 
            val result = "mp:$it"
            externalAssetCache[imageUrl] = result
            result
        }
    }.getOrNull()
}
